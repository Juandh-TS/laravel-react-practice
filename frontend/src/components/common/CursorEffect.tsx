  ...
  useEffect(() => {
    ...
    const spawnTrailDot = (x: number, y: number) => {
      ...
      requestAnimationFrame(() =>
        trailDot.classList.add("cursor-trail-dot--fade"),
      );
    ...
    const spawnStreakBadge = (x: number, y: number, count: number) => {
      ...
      if (count >= STREAK_HOT_THRESHOLD)
        badge.classList.add("cursor-streak--hot");
    ...
    const handleMouseDown = (event: MouseEvent) => {
      const now = performance.now();
      clickStreak =
        now - lastClickTime <= STREAK_RESET_MS ? clickStreak + 1 : 1;
      lastClickTime = now;
    ...
    const drawLaser = () => {
      ...
      laserPoints = laserPoints.filter(
        (point) => now - point.t < LASER_POINT_LIFETIME_MS,
      );
      ...
      for (let i = 1; i < laserPoints.length; i++) {
        ...
        const alpha = Math.max(
          0,
          1 - (now - point.t) / LASER_POINT_LIFETIME_MS,
        );
